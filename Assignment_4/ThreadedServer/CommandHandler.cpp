...
void CommandHandler::handleGet(SOCKET client_socket, std::string& client_name, std::string& file_name) {
	std::filesystem::create_directories(file_directory + "/" + client_name);
	std::ifstream file(file_directory + "/" + client_name + "/" + file_name, std::ios::binary);
...
void CommandHandler::handleList(SOCKET client_socket, std::string& client_name) {
	std::filesystem::create_directories(file_directory + "/" + client_name);
	std::string result;
	try {
		for (const auto& entry : std::filesystem::directory_iterator(file_directory + "/" + client_name)) {
...
void CommandHandler::handlePut(SOCKET client_socket, std::string& client_name, std::string& file_name, uint32_t file_size) {
	std::filesystem::create_directories(file_directory + "/" + client_name);
	std::ofstream file(file_directory + "/" + client_name + "/" + file_name, std::ios::binary);
...
void CommandHandler::handleDelete(SOCKET client_socket, std::string& client_name, std::string& file_name) {
	std::filesystem::create_directories(file_directory + "/" + client_name);
	std::filesystem::path file_path = file_directory + "/" + client_name+ "/" + file_name;
	try {
...
void CommandHandler::handleInfo(SOCKET client_socket, std::string& client_name, std::string& file_name) {
	std::filesystem::create_directories(file_directory + "/" + client_name);
	std::filesystem::path file_path = file_directory + "/" + client_name + "/" + file_name;
