...
std::vector<char> RequestBuilder::serializeClientName(const std::string& client_name)
{
    std::vector<char> client_name_buf;
    uint16_t client_name_length = static_cast<uint16_t>(client_name.length());
    uint16_t net_client_name_length = htons(client_name_length);
    client_name_buf.insert(client_name_buf.end(), reinterpret_cast<char*>(&net_client_name_length),
        reinterpret_cast<char*>(&net_client_name_length) + CLIENT_NAME_LENGTH_BYTES);
    client_name_buf.insert(client_name_buf.end(), client_name.begin(), client_name.end());
...
std::vector<char> RequestBuilder::buildListRequest(const std::string& client_name)
{
    std::vector<char> request_header;
